<script lang="ts">
  ...
  class Game {
    ...
    transformPos([x,y]: Vec2) {
      return [x - this.camera.pos[0], this.zoom - (y - this.camera.pos[1])] as Vec2;
    }
    ...
    play() {
      ...
      const advanceFrame: FrameRequestCallback = (time) => {
        ...
        if(Math.abs(this.user.velocity[0]) > 0.0) {
          this.user.velocity[0] += this.user.velocity[0] * this.friction * delta;
        }
        ...
        this.colliders.forEach((collider) => {
          ...
          const userBox = getBoundingBox({pos: [newX, newY] as Vec2, size: this.user.size})
          
          if(userBox.bottom < colliderBox.top && userBox.top > colliderBox.bottom && userBox.left < colliderBox.right && userBox.right > colliderBox.left){
            // collided 
            if(this.user.velocity[1] < 0 && (userBox.right > colliderBox.left || userBox.left < colliderBox.right ) &&( userBox.top >= colliderBox.top)){
              this.user.velocity[1] = 0;
            ...
            if(this.user.velocity[1] > 0 && (userBox.right > colliderBox.left || userBox.left < colliderBox.right ) &&( userBox.bottom <= colliderBox.bottom)){
              this.user.velocity[1] = 0;
              this.user.movement[1] = 0;
            ...
            else if(this.user.velocity[0] && (userBox.top <= colliderBox.top) && (userBox.left < colliderBox.left || userBox.right > colliderBox.right)){
              this.user.velocity[0] = 0;
        ...
        const rightScroll = (this.camera.pos[0] + this.camera.padding[0]) - this.user.pos[0]
        const leftScroll = this.user.pos[0] - ((100 * this.dx.width / this.dx.height * .5 -  this.camera.padding[0]) + this.camera.pos[0])
        
        const topScroll = this.user.pos[1] - (( 100 - this.camera.padding[1]) + this.camera.pos[1])
        const bottomScroll = (this.camera.pos[1] + ( 60 -this.camera.padding[1])) - this.user.pos[1]
        
        // camera
        if (rightScroll > 0) {
          this.camera.pos[0] -= rightScroll*this.camera.inertia[0];
        }
        else if (leftScroll > 0) {
          this.camera.pos[0] += leftScroll**this.camera.inertia[0];
        }

        if (topScroll > 0) {
          this.camera.pos[1] += topScroll*this.camera.inertia[1];
        }
        else if (bottomScroll > 0) {
          this.camera.pos[1] -= bottomScroll*this.camera.inertia[1] ;
        }
    ...
    render() {
      // draw user
      const pos = this.scale(this.transformPos([this.user.pos[0], this.user.pos[1]]));
      ...
      this.colliders.forEach((collider) => {
        this.dx.ctx.fillRect(...this.scale(this.transformPos(collider.pos)), ...this.scale(collider.size));
      })
    ...
    keyUp(key){
      ...
      if(key === this.user.keyMove)
        this.user.keyMove = undefined;
    }
  }
